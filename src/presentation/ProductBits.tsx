...
export function ProductPrice({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <View style={[styles.priceRow, compact && styles.compactRow]}>
      <Text style={[styles.price, compact && styles.compactPrice]}>${product.price.toFixed(2)}</Text>
      {original !== null && <Text accessibilityLabel={`Before discount: $${original.toFixed(2)}`} style={[styles.original, compact && styles.compactOriginal]}>${original.toFixed(2)}</Text>}
      {!compact && <View style={styles.priceDiscount}><DiscountBadge percentage={discount} /></View>}
    </View>
  );
}
...
export function CartButton({ onPress }: { onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={`Open cart, ${itemCount} items`} style={styles.cartButton}>
      <Ionicons name="cart-outline" size={23} color="#20242B" />
...
const styles = StyleSheet.create({
  ...
  smallDiscount: { backgroundColor: '#EF454A', color: '#FFFFFF', fontSize: 9, borderRadius: 5, paddingHorizontal: 4, paddingVertical: 2 },
  ...
  priceDiscount: { paddingTop: 6 },
  compactRow: { gap: 6 },
  compactPrice: { fontSize: 16, letterSpacing: -0.2 },
  compactOriginal: { fontSize: 12 },
