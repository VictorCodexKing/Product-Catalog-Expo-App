...
jest.mock('../../data/products', () => ({ ...jest.requireActual('../../data/products'), fetchCatalogFacets: jest.fn().mockResolvedValue({ categories: ['beauty', 'groceries'], brands: ['Everyday', 'Travel'], maxPrice: 100 }) }));
jest.mock('@expo/vector-icons/Ionicons', () => () => null);
const mockNavigate = jest.fn();
jest.mock('@react-navigation/native', () => ({ useNavigation: () => ({ navigate: mockNavigate }) }));
jest.mock('../CartContext', () => ({ useCart: () => ({ itemCount: 0, notifications: [] }) }));
...
test('debounces typing and applies category, brand, and price filters, then resets all filters', async () => {
  jest.useFakeTimers();
  try {
