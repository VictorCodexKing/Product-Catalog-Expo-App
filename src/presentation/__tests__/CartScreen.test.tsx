...
const popTo = jest.fn();
const navigation = { goBack, popTo } as unknown as ComponentProps<typeof CartScreen>['navigation'];
...
test('updates quantities and total, stops at stock, then removes the product', async () => {
  ...
  expect(screen.getByText('$19.98')).toBeOnTheScreen();
  await fireEvent.press(screen.getByRole('button', { name: 'Remove Perfume' }));
